...
namespace Squence.Entities
{
    ...
    internal class Bullet(Vector2 bulletPosition, Vector2 direction, BulletType bulletType) : IRenderable, IUpdatable
    {
        public Guid Guid { get; } = Guid.NewGuid();
        public string TextureName { get; } = GetBulletTextureName(bulletType);
        public Vector2 TexturePosition { get => _texturePosition; }
        public float Rotation = (float)Math.Atan2(direction.Y, direction.X);
        ...
        public readonly float _rotation = (float)Math.Atan2(direction.Y, direction.X) + MathF.PI / -2f;
